...
function getNextFriday(date) {
  ...
  for (let i = 0; i < 7; i += 1) {
    if (weekDay === i && i < 5) {
      monthDay += 5 - i;
    } else if (weekDay === i && i === 5) {
      monthDay += i + 2;
    } else if (weekDay === i && i === 6) {
      monthDay += i;
    }
  }
...
function isDateInPeriod(date, period) {
  ...
  if (
    target.getTime() >= date1.getTime() &&
    target.getTime() <= date2.getTime()
  ) {
    return true;
  }
  return false;
...
function formatDate(date) {
  ...
  const minutes =
    obj.getMinutes() > 9 ? obj.getMinutes() : `0${obj.getMinutes()}`;
  let seconds =
    obj.getSeconds() > 9 ? obj.getSeconds() : `0${obj.getSeconds()}`;
  if (hours > 12) {
    hours -= 12;
    seconds = `${seconds} PM`;
  } else if (hours === 12) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const start =
    period.start.slice(3, 6) + period.start.slice(0, 3) + period.start.slice(6);
  const end =
    period.end.slice(3, 6) + period.end.slice(0, 3) + period.end.slice(6);
